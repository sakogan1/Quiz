...
let headerText = document.createTextNode("Quiz")
...
data.forEach((key)=>{
    ...
    quizImage.src=key.url
    document.getElementById("content").appendChild(quizImage)
    let spacing = document.createElement(`br`)
    document.getElementById("content").appendChild(spacing)
    
   
 
    let quizText = document.createElement(`p`)
    quizTextSelect = key.question
    let quizTextContent = document.createTextNode(quizTextSelect)
    quizText.appendChild(quizTextContent)
    document.getElementById("content").appendChild(quizText)


    for(j=0;j<key.choice.length;j++){
  
       
        let quizChoice = document.createElement(`div`)
        quizChoice.id="button"

        let choosen = key.choice[j]
        console.log(key.choice.length)

        let choiceText = document.createTextNode(choosen)
    
        quizChoice.appendChild(choiceText)
     
        
        document.getElementById("content").appendChild(quizChoice)
    
        quizChoice.addEventListener(`click`,()=>{
        if(choosen===key.answer){
